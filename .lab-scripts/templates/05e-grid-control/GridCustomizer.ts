...
export class GridApi {
    ...
    private static customizeItemsGrid(dataset: IDataset): void {
        ...
        dataset.setInterceptor('columns', (columns: IColumn[]) => columns.map(column =>
            column.name === '__PREFIX___availablequantity'
        ...
        dataset.addEventListener('onRecordLoaded', (record: IRecord) => {
            record.expressions?.ui.setCustomFormattingExpression('__PREFIX___availablequantity', () => {
                const quantity = record.getValue('__PREFIX___availablequantity');
                if (quantity == null) return undefined;
                const reorderPoint = (record.getValue('__PREFIX___reorderpoint') as number) ?? LOW_STOCK_FALLBACK;
