...
# Vite + React SPA — in its own solution, so both UI styles ship from the same repo and the
# same package. This completes the inner loop — a working app from source. The Grid control
# package itself is imported into the environment by CP10, before the app package.
#
...
try {
    ...
    . "$PSScriptRoot/scaffold/05f-code-app-solution.ps1"
    . "$PSScriptRoot/scaffold/05g-code-app.ps1"
...
Build the model-driven warehouse experience so users can navigate inventory data from a complete Dataverse app. This adds the app shell along with the core forms, views, scripts, and command surface, swaps the stock items subgrid for the TALXIS Grid PCF customized from our own script library through the control's Client API, and adds a code app for the fully custom parts of the UI.
...
- add src/Solutions.CodeApp with the src/CodeApps.Warehouse code app (Vite + React + TypeScript)
...
## Testing
- dotnet build --nologo --verbosity quiet passes with the UI, TALXIS Grid, and code app solutions included
'@
